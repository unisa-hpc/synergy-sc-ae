...
void advec_cell_kernel(
	...
	if (dir == g_xdir) {
		...
		if (sweep_number == 1) {
			clover::execute($mem_freq, $core_freq, queue, "advec_cell_1", [&](handler &h) {
		...
		clover::execute($mem_freq, $core_freq, queue, "advec_cell_2", [&](handler &h) {
		...
		clover::execute($mem_freq, $core_freq, queue, "advec_cell_3", [&](handler &h) {
