...
void advec_mom_kernel(
	...
	if (mom_sweep == 1) { // x 1
		clover::execute($mem_freq, $core_freq, q,"advec_mom_1",[&](handler &h) {
	...
	if (direction == 1) {
		if (which_vel == 1) {
			...
			clover::execute($mem_freq, $core_freq, q, "advec_mom_2", [&](handler &h) {
			...
			clover::execute($mem_freq, $core_freq, q, "advec_mom_3",[&](handler &h) {
		...
		clover::execute($mem_freq, $core_freq, q, "advec_mom_4", [&](handler &h) {
		...
		clover::execute($mem_freq, $core_freq, q, "advec_mom_5",[&](handler &h) {
