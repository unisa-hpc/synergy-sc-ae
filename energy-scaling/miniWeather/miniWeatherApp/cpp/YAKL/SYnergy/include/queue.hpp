...
public:
  ...
  queue(Args&&... args)
    ...
    device_profiler = std::make_shared<std::future<void>>(std::async(
        std::launch::async,
        detail::coarse_grained_profiler(finished, device)
    ));
  ...
  sycl::event submit(frequency uncore_frequency, frequency core_frequency, T cfg, Rest&&... rest) {
    ...
    if (uncore_frequency != 0 && core_frequency != 0) {
      sycl::event scaling_event = sycl::queue::submit([&](sycl::handler& h) {
        try {
          device.set_all_frequencies(core_frequency, uncore_frequency);
        } catch (const std::exception& e) {
          std::cerr << e.what() << '\n';
        }
      });
...
private:
  ...
  std::shared_ptr<std::future<void>> device_profiler;
#ifdef SYNERGY_ENABLE_PROFILING
