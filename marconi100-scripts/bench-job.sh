...
#SBATCH --ntasks=1                      # tasks out of 128
...
#srun --output=logs/aes_encrypt_${mem_freq}_${core_freq}.log ~/sycl-bench/build/aes_encrypt --device=gpu --size=4096 --num-iters=5 --num-runs=$1 --memory-freq=${mem_freq} --core-freq=${core_freq}
...
#srun --output=logs/scalar_prod_${mem_freq}_${core_freq}.log ~/sycl-bench/build/scalar_prod --device=gpu --size=1048576 --num-iters=100000 --num-runs=$1 --memory-freq=${mem_freq} --core-freq=${core_freq}
...
#srun --output=logs/sobel5_${mem_freq}_${core_freq}.log ~/sycl-bench/build/sobel5  --device=gpu --num-iters=400 --num-runs=$1 --memory-freq=${mem_freq} --core-freq=${core_freq}
#srun --output=logs/sobel7_${mem_freq}_${core_freq}.log ~/sycl-bench/build/sobel7  --device=gpu --num-iters=200 --num-runs=$1 --memory-freq=${mem_freq} --core-freq=${core_freq}
#srun --output=logs/spmv_${mem_freq}_${core_freq}.log ~/sycl-bench/build/spmv --device=gpu --size=524288 --num-iters=2000 --num-runs=$1 --memory-freq=${mem_freq} --core-freq=${core_freq}
